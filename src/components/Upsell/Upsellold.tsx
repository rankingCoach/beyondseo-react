...
export const Upsell = () => {
  ...
  const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>(() =>
    featureCategories.reduce(
      (acc: Record<string, boolean>, category: FeatureCategory) => {
    ...
    if (status.type === "check") {
      return (
        <span className={classNames(styles.statusIcon, styles.statusPositive)}>
          <Icon type={IconSize.small} color="#2bb673" hasCircle fillColor="#e9f7ef">
            {IconNames.check}
          </Icon>
        </span>
      );
    ...
    if (status.type === "cross") {
      return (
        <span className={classNames(styles.statusIcon, styles.statusNegative)}>
          <Icon type={IconSize.small} color="#d14b4b" hasCircle fillColor="#ffefef">
            {IconNames.close}
          </Icon>
        </span>
      );
    ...
    <div className={styles.upsellPage}>
      ...
      >
        <div className={styles.limitedOfferBadge}>
          ...
          {__("Limited Offer: First 3 months only $5/month!", "beyondseo")}
        </div>
        ...
        <Text type={TextTypes.text} className={styles.heroSubtitle}>
          {__(
            "Upgrade to rankingCoach 360 to access all features in your SEO, Online Presence, Social, and Reputation blocks, as well as new ones.",
            "beyondseo",
          )}
        </Text>
        ...
        <div className={styles.periodSwitcher}>
          ...
          <Switch value={paymentPeriod === "annual"} onChange={togglePaymentPeriod} size="small" />
          <span
        ...
        <div className={styles.paymentInfo}>
          {paymentPeriod === "annual" ? __("* Annual payment", "beyondseo") : __("* Monthly payment", "beyondseo")}
        </div>
      ...
      <div className={styles.foundationSection}>
        ...
        <Text type={TextTypes.text} className={styles.foundationSubtitle}>
          {__(
            "The free rankingCoach Radar plugin is a great start. But to continue growing your business, you need the full power.",
            "beyondseo",
          )}
        </Text>
              ...
              <div className={styles.toggleContainer}>
                <div
                  className={classNames(
                    styles.toggleOption,
                    paymentPeriod === "annual" ? styles.toggleOptionActive : "",
                  )}
                  onClick={() => setPaymentPeriod("annual")}
                >
                ...
                <div
                  className={classNames(
                    styles.toggleOption,
                    paymentPeriod === "monthly" ? styles.toggleOptionActive : "",
                  )}
                  onClick={() => setPaymentPeriod("monthly")}
                >
                  {__("Monthly", "beyondseo")}
        ...
        <div className={styles.comparisonContainer}>
          <div className={styles.comparisonHeader}>
            <Text type={TextTypes.heading2} fontWeight={FontWeights.regular}>
              {__("What you get with rankingCoach 360", "beyondseo")}
            </Text>
          ...
          <div className={styles.comparisonCard}>
            <div className={styles.tableHeader}>
              ...
              <Text
                ...
                className={classNames(styles.headerCell, styles.headerCellCenter)}
              >
              ...
              return (
                <div key={category.id} className={styles.categoryBlock}>
                  ...
                  >
                    ...
                    <div className={classNames(styles.cell, styles.statusCell)}>{renderStatus(category.radar)}</div>
                    <div className={classNames(styles.cell, styles.statusCell)}>{renderStatus(category.rc360)}</div>
                  </button>
                    ...
                    <div className={styles.detailsWrapper} id={`feature-${category.id}`}>
                      {category.rows.map((row: FeatureRow, rowIndex: number) => (
                        <div className={classNames(styles.row, styles.detailRow)} key={rowIndex}>
                          ...
                          <div className={classNames(styles.cell, styles.statusCell)}>{renderStatus(row.radar)}</div>
                          <div className={classNames(styles.cell, styles.statusCell)}>{renderStatus(row.rc360)}</div>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div className={styles.legendRow}>
              <div className={styles.legendItem}>
                ...
                <Text type={TextTypes.text} className={styles.legendText}>
                  {__("Full access", "beyondseo")}
                </Text>
              </div>
              <div className={styles.legendItem}>
                <Icon type={IconSize.small} color="#d14b4b" hasCircle fillColor="#ffefef">
        ...
        <div className={styles.featureHighlightsContainer}>
          <div className={styles.featureHighlightsHeader}>
            ...
            <Text type={TextTypes.text} className={styles.featureHighlightsSubtitle}>
              {__(
                "rankingCoach 360 combines everything you need for successful online marketing. Simple, effective, affordable.",
                "beyondseo",
              )}
            </Text>
          </div>
          ...
          <div className={styles.featureHighlightsGrid}>
            {featureHighlights.map((feature: FeatureHighlight) => (
              <div key={feature.id} className={styles.featureHighlightCard}>
                <div className={styles.featureIconWrapper} style={{ backgroundColor: feature.iconBackground }}>
